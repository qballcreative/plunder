...
import { Anchor, Swords, Users, Skull, Ship, Gem, Coins, Wine, CircleDot, Shirt, CloudLightning, Crosshair, Gift } from 'lucide-react';
...
export const LandingPage = () => {
  ...
  const toggleRule = (key: keyof OptionalRules) => {
    setOptionalRules(prev => ({ ...prev, [key]: !prev[key] }));
  };
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <section className="relative flex-1 flex items-center justify-center p-4">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(3)].map((_, i) => (
            <motion.div
              ...
              initial={{ x: `${20 + i * 30}%`, y: `${10 + i * 25}%`, rotate: Math.random() * 30 }}
              animate={{ y: [null, `${5 + i * 20}%`], rotate: [null, Math.random() * 30 - 15] }}
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          >
            <div className="flex items-center justify-center gap-4 mb-4">
              <motion.div animate={{ rotate: [0, -10, 10, 0] }} transition={{ duration: 4, repeat: Infinity }}>
                <Skull className="w-12 h-12 lg:w-16 lg:h-16 text-primary drop-shadow-lg" />
              </motion.div>
            </div>
          ...
          {/* Goods showcase */}
          <motion.div
            className="flex items-center justify-center gap-4 lg:gap-6 my-8 flex-wrap"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          {/* Game setup */}
          <motion.div
            className="bg-card/90 backdrop-blur-md rounded-2xl p-6 lg:p-8 border border-primary/30 shadow-2xl max-w-md mx-auto"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="mb-6">
              ...
              <Input
                ...
                placeholder="Captain..."
                className="bg-muted/50 border-primary/30 focus:border-primary text-center text-lg"
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        difficulty === level ? config.color : 'border-border hover:border-primary/30 bg-muted/30'
                      )}
                    >
                ...
                {optionalRulesConfig.map((rule) => (
                  <button
                    ...
                    className={cn(
                      ...
                      optionalRules[rule.key] ? rule.color : 'border-border hover:border-primary/30 bg-muted/30'
                    )}
                  >
                    ...
                    <div className={cn(
                      ...
                      optionalRules[rule.key] ? 'bg-current border-current' : 'border-muted-foreground'
                    )}>
          ...
          >
            <p className="mb-2">Based on the award-winning Jaipur card game</p>
      ...
      <section className="relative py-12 px-4 bg-card/80 backdrop-blur-sm border-t border-border">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: <Ship className="w-10 h-10" />, title: 'Take', description: 'Take a card from the market or take all ships at once to grow your fleet.' },
              { icon: <Swords className="w-10 h-10" />, title: 'Exchange', description: 'Swap 2+ cards between your hand and the market. Use ships as wildcards!' },
              { icon: <Coins className="w-10 h-10" />, title: 'Sell', description: 'Sell matching cards for treasure tokens. More cards means bigger bonus rewards!' }
            ].map((item, i) => (
              <motion.div
                key={item.title}
                className="p-6 rounded-xl bg-muted/50 border border-border text-center hover:border-primary/30 transition-colors"
